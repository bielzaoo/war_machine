    ...
    ( set +e
      while [[ -f "$flag" ]]; do
          for frame in "${frames[@]}"; do
              [[ -f "$flag" ]] || break
              printf "
  ${CYAN}%s${RESET} %s  " "$frame" "$msg" > /dev/tty
              sleep 0.08
          done
      done
      # Limpa a linha do spinner antes de sair — sem 
 residual no terminal.
      printf "
%-72s
" "" > /dev/tty
    ) &
    # $! é capturado aqui, no escopo do pai — não dentro de $().
    _SPIN_PID=$!
}
...
stop_spinner() {
    ...
    # wait garante que o subshell terminou de fato e não há printf em voo.
    # Sem isso, a linha de resultado poderia aparecer antes do último frame.
...
" > /dev/tty
        ...
        printf "  ${YELLOW}⏭${RESET} Skipped by user.
" > /dev/tty
    else
        printf "  ${RED}✘${RESET} Failed or no results.
" > /dev/tty
    fi
}
