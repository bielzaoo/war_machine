...
start_spinner() {
    ...
    local frames=('⠋' '⠙' '⠹' '⠸' '⠼' '⠴' '⠦' '⠧' '⠇' '⠏')
    # [SEC] Escreve diretamente em /dev/tty em vez de stdout/stderr.
    #       /dev/tty é o terminal real do processo, independentemente de
    #       redireções em pipes internos — elimina a condição de corrida
    #       onde frames residuais apareciam misturados com linhas de log.
    #       set +e isola erros de printf/sleep do processo pai (set -e ativo).
    ( set +e
      while true; do
...
stop_spinner() {
    local pid=$1
    local status=${2:-0}
    # Mata e aguarda o spinner *antes* de imprimir a linha de resultado,
    # garantindo que nenhum frame residual apareça depois do log.
    kill "$pid" 2>/dev/null || true
    wait "$pid" 2>/dev/null || true
    # Grava em /dev/tty para consistência com start_spinner.
    if [[ "$status" -eq 0 ]]; then
...
run_tool() {
    ...
    # [TIMING] log_tool é chamado ANTES do spinner para que "Starting..." apareça
    #          limpo no terminal. O spinner só inicia depois, na linha seguinte.
    #          Isso elimina a condição de corrida onde o "Starting..." da próxima
    #          tool aparecia enquanto o spinner da anterior ainda estava ativo.
    ...
    while [[ $attempt -lt $MAX_RETRIES ]]; do
        ...
        # Pequeno sleep antes de lançar garante que qualquer flush de /dev/tty
        # do stop_spinner anterior seja concluído antes de escrever novos frames.
        sleep 0.05
