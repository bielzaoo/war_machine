...
run_tool() {
  ...
  local err_log="/tmp/rataria_err_${tool_name//[^a-zA-Z0-9]/_}.log"
  local attempt=0
  local success=0
  ...
  while [[ $attempt -lt $MAX_RETRIES ]]; do
    ...
    # Executa o comando suprimindo toda saída avulsa:
    #   - stdout do processo → /dev/null  (a saída útil já vai pro arquivo via > no cmd)
    #   - stderr             → log de erro em /tmp para debug posterior
    # Ferramentas que ignoram -silent ou despejam progresso no stderr ficam silenciosas.
    eval "$cmd" >/dev/null 2>"$err_log"
    local exit_code=$?
  ...
  if [[ $success -eq 0 ]]; then
    log_error "${tool_name} falhou após ${MAX_RETRIES} tentativas. Veja: ${err_log}"
